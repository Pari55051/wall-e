...
void playAudio() {
  ...
  if (!isAudioPlaying) {
    ...
    audioFile = SD.open("/wall-e.wav", FILE_READ);
    if (!audioFile) {
...
void findEva() {
  ...
  if (receivedData.button3State && !previousButton3State) {
    ...
    if (!isAudioPlaying) {
      ...
      audioFile = SD.open("/eva.wav", FILE_READ);
      if (!audioFile) {
        Serial.println("Failed to open audio file!");
        return;
      }
    ...
    for (int i = 0; i < 2; i++) {  // Perform 3 cycles of neck movement
      ...
      for (int pos = 105; pos >= 75; pos--) {
        neckSidewaysServo.write(pos);
        delay(30);  // Delay between movements
      }
      delay(500);

      for (int pos = 75; pos <= 95; pos++) {
        neckSidewaysServo.write(pos);
        delay(30);  // Delay between movements
      }
      delay(500);
...
void handleNeckMovement() {
  ...
  if (receivedData.button2State && !previousButton2State) {
    ...
    delay(300);  // Wait for servos to reach their neutral position
...
void performDance() {
  ...
  if (receivedData.button4State && !previousButton4State) {
    ...
    if (!isAudioPlaying) {
      ...
      audioFile = SD.open("/dance.wav", FILE_READ);
      if (!audioFile) {
        Serial.println("Failed to open audio file!");
        return;
      }
